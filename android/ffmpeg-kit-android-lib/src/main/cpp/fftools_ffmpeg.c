...
/* FFmpegKit: report callback for statistics */
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    int ret;
    float t;
    uint64_t ffmpegkit_frame_number = 0;
    float ffmpegkit_fps = 0;
    float ffmpegkit_q = 0;
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        ...
        if (!vid && ost->type == AVMEDIA_TYPE_VIDEO) {
            ...
            ffmpegkit_frame_number = frame_number;
            ffmpegkit_fps = fps;
    ...
    /* FFmpegKit: forward report to callback */
    forward_report(ffmpegkit_frame_number, ffmpegkit_fps, ffmpegkit_q, total_size, pts, bitrate, speed);
